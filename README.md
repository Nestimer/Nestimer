...
2. Каждые 30 секунд агент проверяет активность, считает время **только когда**:
   - Экран включён (не в sleep)
   - Экран **не заблокирован** (пользователь залогинен, не на lock screen) — через `CGSessionCopyCurrentDictionary`
   - Пользователь **не idle** (была активность мыши/клавиатуры в последние 5 мин) — через `IOHIDSystem` idle time
